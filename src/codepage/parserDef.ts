...
CP0Auto.insertHex('82', 8, (slice) => {
    ...
    return { code: 'PUSHINT', args: [new BN(x.toString(10))] };
})
...
// 8781824 (DUMMY)
CP0Auto.insertHex('88', 8, { code: 'PUSHREF' });
CP0Auto.insertHex('89', 8, { code: 'PUSHREFSLICE' });
CP0Auto.insertHex('8a', 8, { code: 'PUSHREFCONT' });
CP0Auto.insertHex('8b', 8, (slice) => {
    let x = slice.readUintNumber(4);
    let len = 8 * x + 4;
    let subslice = fetchSubslice(slice, len);
    return { code: 'PUSHSLICE', args: [subslice.toCell()] };
});
...
CP0Auto.insertHex('8E', 7, (slice) => {
    ...
    return { code: 'PUSHCONT', args: [subslice.toCell()] };
})
...
CP0Auto.insertHex('cf8', 9, (slice) => {
    ...
    return { code: `STSLICECONST`, args: [subslice.toCell()] };
});
...
// •D704—LDIXQ(sl–xs′ −1ors0),quietversionofLDIX:loadsa signed l-bit integer from s similarly to LDIX, but returns a success flag, equal to −1 on success or to 0 on failure (if s does not have l bits), instead of throwing a cell underflow exception.
// • D705—LDUXQ(sl–xs′ −1ors0),quietversionofLDUX.
...
//     return { code: `${longerVersion ? ((slice.readUintNumber(8) + 1) + ' ') : ''}${preload ? 'PLD' : 'LD'}${sign ? 'U' : 'I'}${quiet ? 'Q' : ''}` };
// });
CP0Auto.insertHex('d710', 13, (slice) => {
    let c = slice.readUintNumber(3) + 1;
...
CP0Auto.insertHex('db3c', 16, (slice) => {
    ...
    return { code: 'CALLREF', args: [subslice.toCell()] };
});
CP0Auto.insertHex('db3d', 16, (slice) => {
    let subslice = slice.readRef();
    return { code: 'JMPREF', args: [subslice.toCell()] };
});
CP0Auto.insertHex('db3e', 16, (slice) => {
    let subslice = slice.readRef();
    return { code: 'JMPREFDATA', args: [subslice.toCell()] };
});
...
CP0Auto.insertHex('e300', 16, (slice) => {
    let subslice = slice.readRef();
    return { code: 'IFREF', args: [subslice.toCell()] };
});
CP0Auto.insertHex('e301', 16, (slice) => {
    let subslice = slice.readRef();
    return { code: 'IFNOTREF', args: [subslice.toCell()] };
});
CP0Auto.insertHex('e302', 16, (slice) => {
    let subslice = slice.readRef();
    return { code: 'IFJMPREF', args: [subslice.toCell()] };
});
CP0Auto.insertHex('e303', 16, (slice) => {
    let subslice = slice.readRef();
    return { code: 'IFNOTJMPREF', args: [subslice.toCell()] };
});
...
CP0Auto.insertHex('e30d', 16, (slice) => {
    let subslice = slice.readRef();
    return { code: 'IFREFELSE', args: [subslice.toCell()] }
});
CP0Auto.insertHex('e30e', 16, (slice) => {
    let subslice = slice.readRef();
    return { code: 'IFELSEREF', args: [subslice.toCell()] }
});
CP0Auto.insertHex('e30f', 16, (slice) => {
    let subslice = slice.readRef();
    return { code: 'IFREFELSEREF', args: [subslice.toCell()] }
});
// 14880768 (DUMMY)
...
CP0Auto.insertHex('f20', 10, (slice) => {
    let nn = slice.readUintNumber(6);
    return { code: 'THROW', args: [nn] };
});
CP0Auto.insertHex('F24', 10, (slice) => {
    let eCode = slice.readUintNumber(6)
...
CP0Auto.insertHex('f48', 11, (slice) => {
    let remove = slice.readBit();
    let max = slice.readBit();
    let int = slice.readBit();
    let usign = slice.readBit();
    let ref = slice.readBit();
...
CP0Auto.insertHex('f4a0', 13, (slice) => {
    ...
    if (push) {
        ...
        return { code: 'DICTPUSHCONST', args: [keyLen, subslice.readRef().toCell()] };
    }
